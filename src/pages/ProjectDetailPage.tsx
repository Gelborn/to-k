...
export const ProjectDetailPage: React.FC = () => {
  ...
  const EmptyState: React.FC<{ 
    ...
    <div className="flex flex-col items-center justify-center py-16 px-4">
      ...
      <p className="text-gray-400 text-center mb-8 max-w-md font-medium">{description}</p>
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="animate-fade-in">
        {activeTab === 'assets' && (
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <h2 className="text-xl font-bold text-gray-100 tracking-tight">Project Assets</h2>
            ...
            {assets.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {assets.map((asset) => (
                  <div key={asset.id} className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6 hover:border-gray-700/50 transition-all duration-200 group">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full mt-1 ${
                            asset.type === 'unique' 
                              ? 'text-purple-300 bg-purple-900/30 border border-purple-800/50' 
                      ...
                      <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="p-1.5 text-gray-400 hover:text-gray-100 hover:bg-gray-800/50 rounded-lg transition-colors">
                          <Edit3 className="w-4 h-4" />
                        </button>
                        <button className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-900/20 rounded-lg transition-colors">
        ...
        {activeTab === 'tags' && (
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <h2 className="text-xl font-bold text-gray-100 tracking-tight">Project Tags</h2>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-800/50">
                      <tr>
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-300 uppercase tracking-widest">
                          Public ID
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-300 uppercase tracking-widest">
                          Status
                        ...
                        <th className="px-6 py-4 text-right text-xs font-semibold text-gray-300 uppercase tracking-widest">
                          Actions
                        </th>
                      </tr>
                    </thead>
                      ...
                      {tags.map((tag) => (
                        <tr key={tag.id} className="hover:bg-gray-800/30 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              <code className="text-sm font-mono text-gray-100 bg-gray-800/50 px-2 py-1 rounded-lg">
                                {tag.public_id}
                              </code>
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                              tag.status === 'active' 
                                ...
                                : tag.status === 'claimed'
                                ? 'text-blue-300 bg-blue-900/30 border border-blue-800/50'
                                : 'text-gray-300 bg-gray-800/30 border border-gray-700/50'
                            }`}>
                              {tag.status}
                            </span>
                          </td>
        ...
        {activeTab === 'settings' && (
          <div className="space-y-6">
            <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-2xl p-6">
              <h3 className="text-lg font-bold text-gray-100 mb-4 tracking-tight">Project Settings</h3>
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    value={project.type}
                    disabled
                    className="w-full px-4 py-3 bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/50 opacity-50"
                  >
            ...
            <div className="bg-red-900/20 backdrop-blur-sm border border-red-800/50 rounded-2xl p-6">
              <h3 className="text-lg font-bold text-red-400 mb-2 tracking-tight">Danger Zone</h3>
              <p className="text-gray-400 text-sm mb-4 font-medium">
                Once you delete a project, there is no going back. Please be certain.
              </p>
              <Button variant="secondary" className="text-red-400 border-red-800/50 hover:bg-red-900/30">
